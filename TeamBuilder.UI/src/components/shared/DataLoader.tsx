...
export const DataLoader: FunctionComponent = () => {
  ...
  const loading =
    playerState.requestState === "loading" ||
    ...
    matchResumeState.requestState === "loading";
  ...
  const loadResumableMatches = () => {
    dispatch(loadResumableMatchesRequest(matchResumeState.queryFilter));
  };
  ...
  if (!authState.userRestored) {
    const storedUserData = localStorage.getItem(
      AuthProperties.LocalStorageUserKey
    );
    try {
      if (storedUserData) {
  ...
  if (matchResumeState.requestState === "required" && !loading) {
    loadResumableMatches();
  }
  ...
  if (
    authState.user?.roles.includes(Roles.Admin) &&
  ...
  if (playerState.requestState === "error") {
    elements.push(
      <div
        key="reload-players"
        className="size-full flex flex-row justify-center items-center"
      >
        <button onClick={loadPlayers} className="button">
          Reload&nbsp;Players
        </button>
      </div>
    );
  ...
  if (groupState.requestState === "error") {
    elements.push(
      <div
        ...
        className="size-full flex flex-row justify-center items-center"
      >
        <button onClick={loadGroups} className="button">
          Reload&nbsp;Groups
        </button>
      </div>
    );
  ...
  if (matchState.matchesRequestState === "error") {
    elements.push(
      <div
        ...
        className="size-full flex flex-row justify-center items-center"
      >
        <button onClick={loadMatches} className="button">
          Reload&nbsp;Matches
        </button>
      </div>
    );
  ...
  if (matchResumeState.requestState === "error") {
    elements.push(
      <div
        key="reload-resumeable-matches"
        className="size-full flex flex-row justify-center items-center"
      >
        <button onClick={loadResumableMatches} className="button">
          Reload&nbsp;Resumable&nbsp;Matches
        </button>
      </div>
    );
  }
  ...
  if (groupMembersState.memberRequestState === "error") {
    elements.push(
      <div
        key="reload-group-members"
        className="size-full flex flex-row justify-center items-center"
      >
        <button onClick={loadGroupMembers} className="button">
  ...
  return (
    <div className="text-sm absolute w-0 bottom-0 flex flex-col pb-4 left-1/2">
      <div>{elements}</div>
    </div>
  );
};
