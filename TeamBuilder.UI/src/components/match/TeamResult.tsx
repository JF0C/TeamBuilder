...
export const TeamResult: FunctionComponent<{
  index: number;
  team: TeamEntity;
}> = ({ index, team }) => {
  ...
  const setWinner = () => {
    ...
    for (const losingTeam of currentMatch.teams.filter(
      (t, i) => i !== index && t.score >= winningScore
    )) {
      dispatch(
        setTeamScore({
  ...
  return (
    <div className="flex flex-col w-full border border-2 rounded-md p-2">
      <div className="w-full flex flex-row justify-between">
        ...
        <div className="flex flex-row gap-2">
          <input
            ...
            type="number"
            value={team.score}
            onChange={changeTeamScore}
          />
          <button onClick={setWinner}>Has Won</button>
        </div>
      </div>
      <div className="flex flex-row flex-wrap text-msm gap-2">
        {team.players.map((p) => (
          <ListItem key={"team-member-" + p.id}>{p.name}</ListItem>
        ))}
      </div>
    </div>
  );
